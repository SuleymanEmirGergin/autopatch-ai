...
export default async function handler(
  ...
  try {
    ...
    let endpoint = `${BACKEND_URL}/api/recommendations`;
    
    if (path.includes("/priority")) {
      endpoint = `${BACKEND_URL}/api/recommendations/priority`;
    } else if (path.includes("/patches")) {
      endpoint = `${BACKEND_URL}/api/recommendations/patches`;
    }
