...
export default async function handler(
  ...
  try {
    ...
    let endpoint = `${BACKEND_URL}/api/reports/executive-summary`;
    
    if (path.includes("/html")) {
      endpoint = `${BACKEND_URL}/api/reports/executive-summary/html`;
    } else if (path.includes("/markdown")) {
      endpoint = `${BACKEND_URL}/api/reports/executive-summary/markdown`;
    }
