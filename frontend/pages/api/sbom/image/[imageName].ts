...
export default async function handler(
  ...
  try {
    ...
    const encoded = encodeURIComponent(imageName as string);
    const endpoint = req.url?.includes("/rescan")
      ? `${BACKEND_URL}/api/sbom/image/${encoded}/rescan`
      : `${BACKEND_URL}/api/sbom/image/${encoded}`;
